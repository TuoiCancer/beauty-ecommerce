...
export async function POST(req: NextRequest, res: NextResponse) {
	if (req.method === 'POST') {
		...
		Order: 
		<span
        style="font-weight: 600; color: #46da2d;"> DELIVERED  
				</span>
		`
		pUsername.innerHTML = `
		User: <span
        style="font-weight: 600; color: #000; font-size:16px;">${username}</span>
		`

		pPrice.innerHTML = `
		Total: <span style="font-weight: 600; color: #000; font-size:16px;">${order.order_checkout.totalPrice}đ</span>
		`

		pAddress.innerHTML = `
		Address: <span style="font-weight: 600; color: #000;">${order.order_shipping}</span>
		`
		...
		try {
			await transporter.sendMail({
				...
				subject: `ORDER DELIVERIED SUCCESSFULLY`, // Subject line
